...
namespace SchoolScheduleLibrary.Repository
{
    public class RedisRepository : IRedisRepository
    {
        ...
        public async Task<SessionData> GetSessionDataFromKey(string sessionKey)
        {
            ...
            if ( value.IsNullOrEmpty) throw new HttpResponseException(ServiceReturnCode.NotFound, "This session key does not exist!");
            else if (value.HasValue)
            {
                SessionData? session = JsonSerializer.Deserialize<SessionData>((string)value!);
                if (session != null) return session;
                else throw new HttpResponseException(ServiceReturnCode.NoContent, "Session key exists but could not deserialize value!");
            }
            else throw new HttpResponseException(ServiceReturnCode.NoContent, "Session key exists but no value exists!");
